...
logger = logging.getLogger("AuraPostureBackend")
    ...
    try:
        while True:
            ...
            if action == "frame" or action == "calibrate":
                ...
                # Key joints to inspect (MediaPipe landmark indices: Nose=0, LEar=7, REar=8, LShoulder=11, RShoulder=12)
                nose = landmarks[0]
                l_ear = landmarks[7]
                r_ear = landmarks[8]
                ...
                all_visible = all(joint.visibility > 0.5 for joint in required_joints)
                
                # Prepare coordinates to return to client for custom SVG skeleton drawing
                # We normalize back to 0-1 coordinate space
                landmarks_dict = {
                ...
                if calibration_data:
                    ...
                    raw_score = (head_ratio * 0.7 + vertical_sinking_ratio * 0.3) * 100
